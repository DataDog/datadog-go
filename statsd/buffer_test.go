...
func TestBufferGauge(t *testing.T) {
	...
	err := buffer.writeGauge("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, noTimestamp, true)
	assert.Nil(t, err)
	...
	// with a timestamp
	buffer = newStatsdBuffer(1024, 1)
	err = buffer.writeGauge("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, 1658934092, true)
	assert.Nil(t, err)
...
func TestBufferCount(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeCount("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, noTimestamp, true)
	assert.Nil(t, err)
	...
	// with a timestamp
	buffer = newStatsdBuffer(1024, 1)
	err = buffer.writeCount("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, 1658934092, true)
	assert.Nil(t, err)
...
func TestBufferHistogram(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeHistogram("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, true)
	assert.Nil(t, err)
...
func TestBufferDistribution(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeDistribution("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, true)
	assert.Nil(t, err)
...
func TestBufferSet(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeSet("namespace.", []string{"tag:tag"}, "metric", "value", []string{}, 1, true)
	assert.Nil(t, err)
...
func TestBufferTiming(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeTiming("namespace.", []string{"tag:tag"}, "metric", 1, []string{}, 1, true)
	assert.Nil(t, err)
...
func TestBufferEvent(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeEvent(&Event{Title: "title", Text: "text"}, []string{"tag:tag"}, true)
	assert.Nil(t, err)
...
func TestBufferServiceCheck(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	err := buffer.writeServiceCheck(&ServiceCheck{Name: "name", Status: Ok}, []string{"tag:tag"}, true)
	assert.Nil(t, err)
...
func TestBufferAggregated(t *testing.T) {
	buffer := newStatsdBuffer(1024, 1)
	pos, err := buffer.writeAggregated([]byte("h"), "namespace.", []string{"tag:tag"}, "metric", []float64{1}, "", 12, -1, 1, true)
	assert.Nil(t, err)
	assert.Equal(t, 1, pos)
	assert.Equal(t, "namespace.metric:1|h|#tag:tag\n", string(buffer.bytes()))

	buffer = newStatsdBuffer(1024, 1)
	pos, err = buffer.writeAggregated([]byte("h"), "namespace.", []string{"tag:tag"}, "metric", []float64{1, 2, 3, 4}, "", 12, -1, 1, true)
	assert.Nil(t, err)
	...
	buffer = newStatsdBuffer(1024, 1)
	pos, err = buffer.writeAggregated([]byte("h"), "namespace.", []string{"tag:tag"}, "metric", []float64{1, 2, 3, 4}, "", 12, -1, 0.33, true)
	assert.Nil(t, err)
	assert.Equal(t, 4, pos)
	...
	// first value too big
	buffer = newStatsdBuffer(30, 1)
	pos, err = buffer.writeAggregated([]byte("h"), "namespace.", []string{"tag:tag"}, "metric", []float64{12, 2, 3, 4}, "", 12, -1, 1, true)
	assert.Equal(t, errBufferFull, err)
